...
async def login_or_register(user: UserCreate):
    ...
    try:
        db = get_db()
        coll = db.users
        existing = await coll.find_one({"email": user.email.lower()})
        if existing:
...
async def get_me(x_user_email: str = Header(..., alias="X-User-Email")):
    ...
    try:
        db = get_db()
        user = await db.users.find_one({"email": x_user_email.lower()})
        if user:
...
async def get_settings(x_user_email: str = Header(..., alias="X-User-Email")):
    ...
    try:
        ...
        user = await db.users.find_one({"email": x_user_email.lower()})
        if user:
            return user.get("settings", {"api_url": "http://localhost:8000", "default_device": "esp32-demo-1"})
    except Exception:
        pass
    raise HTTPException(status_code=404, detail="User not found")
